...
implementation{
   ...
   event void Boot.booted(){
	  ...
	  call Timer0.startPeriodic(350000);
   ...
   event message_t* Receive.receive(message_t* msg, void* payload, uint8_t len){
      ...
      if(len==sizeof(pack)){
        ...
        if(myMsg->TTL != 0 && !checkSentList(myMsg)){ 
			...
			if(myMsg->seq == 999 && call neighborMap.size()!= 19){
				d//bg(NEIGHBOR_CHANNEL, "Node %d is a neighbor of Node %d  \n" , TOS_NODE_ID, myMsg->src);
			...
			else if(myMsg->dest == AM_BROADCAST_ADDR && call neighborMap.size()!= 19){
